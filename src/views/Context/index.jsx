/*
 * @LastEditTime: 2022-08-06 16:53:09
        ...
        <div>
            ...
            <div>
                ...
                <div>
                    ...
                    <p className="primary">
                        这里主要涉及api-createContext,useContext
                    </p>
                    <div>
            ...
            <ThemeContext.Provider value={"light"}>
              <Toolbar />
                ...
                <div>
                    ...
                    <p>
                        比如，考虑这样一个 Page 组件，它层层向下传递 user 和
                        avatarSize 属性，从而深度嵌套的 Link 和 Avatar
                        组件可以读取到这些属性：
                    </p>
                    <div className="tip-content">
...
{props.userLink}`}</code>
            ...
            <div>
                ...
                <div>
                    ...
                    <div>
                        ...
                        <p className="tip-content">
                            ...
                            Context
                            对象的组件，这个组件会从组件树中离自身最近的那个匹配的
                            Provider 中读取到当前的 context
                            值。只有当组件所处的树中没有匹配到 Provider 时，其
                            defaultValue 参数才会生效。这有助于在不使用 Provider
                    ...
                    <div className="tip-success">
                        ...
                        <div>
                            ...
                            <div>
                                <h3>
                                    现在使用为SayHello提供Provider，并提供vlaue
                                </h3>
                                <div>
                                    {/* 传递undefined不生效 */}
                ...
                <div>
                    ...
                    <p className="tip-content">
                        Provider 接收一个 value 属性，传递给消费组件。一个
                    ...
                    <p className="tip-content">
                        ...
                        及其内部 consumer 组件都不受制于 shouldComponentUpdate
                        函数，因此当 consumer
                    ...
                    <div>
                        ...
                        <div>
                            <MessageContext.Provider
                                value={{ content: 'outContext' }}
                            >
                                <MessageContext.Provider
                                    value={{
                                        forMan: 'test',
                                        content:
                                            '多个Provider嵌套使用-innerContext',
                    ...
                    <div className="primary">
                        <p className="tip-error">
                            ...
                            identity）来决定何时进行渲染，这里可能会有一些陷阱，当
                            provider 的父组件进行重渲染时，可能会在 consumers
                            组件中触发意外的渲染。举个例子，当每一次 Provider
                            重渲染时，以下的代码会重渲染所有下面的 consumers
                            组件，因为 value 属性总是被赋值为新的对象
                        </p>
                        <div className="tip-content">
                            <code>
                                {' '}
...
MyClass.contextType = MyContext;`}</code>
                    ...
                    <p className="tip-error">
                        {' '}
                        挂载在 class 上的 contextType 属性可以赋值为由
                        React.createContext() 创建的 Context
                        对象。此属性可以让你使用 this.context 来获取最近 Context
                        上的值。你可以在任何生命周期中访问到它，包括 render
                        函数中。
                    </p>
                    <p className="tip-success">
                        只通过该 API 订阅单一 context。订阅多个，阅读使用多个
                        Context 章节 使用实验性的 public class fields
                        语法，你可以使用 static 这个类属性来初始化你的
                        contextType。
                    </p>
                    <div className="tip-content">
                        <code>{`class MyClass extends React.Component {
...
`}</code>
  ...
  {value => /* 基于 context 值进行渲染*/}
                    ...
                    <p className="tip-success">
                        ...
                        节点。传递给函数的 value 值等价于组件树上方离这个
                        context 最近的 Provider 提供的 value 值。如果没有对应的
                ...
                <p className="primary">
                    context 对象接受一个名为 displayName 的
                    property，类型为字符串。React DevTools 使用该字符串来确定
                    context 要显示的内容。
                </p>
...
<MyContext.Consumer> // "MyDisplayName.Consumer" 在 DevTools 中`}
            ...
            <div>
                ...
                <p className="tip-error">
                    为了确保 context 快速进行重渲染，React 需要使每一个
                    consumers 组件的 context 在组件树中成为一个单独的节点。
                    如果两个或者更多的 context
